...
def choose_graph_algorithm(adj_list, algo="auto"):
    ...
    # Detect if graph is weighted
    weighted = any(
        isinstance(neighbors[0], (tuple, list)) and len(neighbors[0]) == 2
        for neighbors in adj_list.values() if neighbors
    )

    # Normalize to weighted format if needed
    if weighted:
        for u in adj_list:
    ...
    if algo == "auto":
        ...
        if num_nodes <= 10:
            ...
            return {
                "algorithm": "bfs",
                "features": features,
                "output": bfs(adj_list, 0)
            }

    # Specific algorithm handlers
    elif algo == "bfs":
        ...
        return {"algorithm": "cycle_detection", "features": features, "output": has_cycle_flag}
        ...
        # Create weighted copy for dijkstra algorithms
        ...
        return {
            ...
            "output": {
                ...
                "dijkstra_heap": dijkstra_heap(copy.deepcopy(weighted_adj_list), 0),
                "dijkstra_matrix": dijkstra_matrix(copy.deepcopy(weighted_adj_list), 0),
                "union_find_cycle": run_union_cycle_check(copy.deepcopy(adj_list), num_nodes),
